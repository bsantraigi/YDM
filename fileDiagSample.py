...
def update_bar(parserThread, tkRoot):
    global linkArr, downListChecks
    if not progressQ.empty():
        p = progressQ.get()
        pb["value"] = p
        if p == 100:
            # Create a checklist for videos
            linkArr = q.get()
            i = 0
            for l in linkArr:
                iv = IntVar()
                ...
                c = Checkbutton(tkRoot, text = l['title'], variable=iv)
                c.grid(column=1, row=6 + i, sticky=W)
                i = i+1

            parserThread.join()
            print("Parsing Complete")
            return
    root.after(100, update_bar, parserThread, tkRoot)

# Checkboxes
def printChecks(*args):
    print(downListChecks[0].get())
    print(args)
...
def process():
    ...
    if(url != ""):
        ...
        parserThread.start()        
        update_bar(parserThread, mainframe)
    else:
...
# Default Path:
listURL.set("https://www.youtube.com/playlist?list=PLnKw1txyYzRn2ZZAaL-FC7QrCl66XAylw")

# Default Save path
# savePath.set(os.getcwd())
savePath.set("C:/Users/Bishal/Shared/Youtube/Animating Text - Autodesk Official")
